        ...
        <ol className="mt-16 flex flex-col">
          {roles.map((role, index) => (
            <Reveal key={`${role.company}-${role.period}`} delay={index * 80}>
              <li className="relative grid gap-6 border-t border-[var(--line)] py-10 lg:grid-cols-[minmax(0,15rem)_minmax(0,1fr)] lg:gap-14">
                {/* Meta rail */}
          ...
          <Reveal delay={roles.length * 80}>
            <li className="grid gap-6 border-y border-[var(--line)] py-10 lg:grid-cols-[minmax(0,15rem)_minmax(0,1fr)] lg:gap-14">
              <div className="lg:pt-1">
                <h3 className="font-[family-name:var(--font-display)] text-[1.25rem] font-semibold tracking-[-0.015em] text-[var(--text-primary)]">
                  {education.school}
                </h3>
